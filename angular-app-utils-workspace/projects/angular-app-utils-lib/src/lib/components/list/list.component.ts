...
import {
  DATA_REFRESH_SERVICE_TAG,
  DataRefreshItem,
  DataRefreshService,
  DATA_REFRESH_SERVICE_NEXT_TAG,
} from "../../services/data-refresh.service";
...
} from "./../../api-datasource/api-datasource";
...
} from "../stampa/stampa-modal/stampa-modal.component";
import {
  CampoStampaInterface,
  getPrintFormat,
  StampaFormConfig,
  StampaModalResponse,
} from "../stampa/stampa.model";
...
@Directive()
export abstract class ListComponent<T, LoginInfo>
  ...
  implements OnInit, AfterViewInit
{
  ...
  ngOnInit(): void {
    ...
    this.sub.add(
      this.dataRefreshService.refresh.subscribe((res: DataRefreshItem) => {
        this.refreshFromService(res);
      })
    );
    this.sub.add(
      this.dataRefreshService.nextDetail.subscribe((res: DataRefreshItem) => {
  ...
  setupPaginatorAndSort() {
    ...
    if (this.paginator) {
      ...
      if (this.sort) {
        ...
        this.dataSub.add(
          this.paginator.page.subscribe(() => this.loadListData())
        );
      }
    }
  }
  ...
  refreshFromService(res: DataRefreshItem) {
    if (res && res.IdentifierName === this.LIST_NAME) {
      if (
        this.refreshAll ||
        (res.ElementUpdatedId == null && res.ElementUpdated == null)
      ) {
      ...
      if (id) {
        ...
        if (action == ApiActionsType.DeleteAction) {
          ...
          if (this.dataSource instanceof MatTableDataSource) {
            ...
            //caso lista generica
            this.dataSource = this.refreshItemListDeleted(
              el,
              id,
              this.dataSource
            );
        ...
        console.log(
          "ListComponent",
          "Impossibile ricaricare il dettaglio nella lista. Manca id"
        );
      }
    } else {
  ...
  refreshItemListAdded(el: T, list: T[]): boolean {
    if (list) {
      const itemToUpdateIndex = list.findIndex(
        (item) => this.idExtractor(item) == null
      );
  ...
  refreshItemListDeleted(el: T, id: any, list: T[]): T[] {
    if (list) {
      list = list.filter((item: T) => {
        return (
          this.idExtractor(item) != (id != null ? id : this.idExtractor(el))
        );
      });
      for (let i = 0; i < list.length; i++) {
        const item = list[i];
  ...
  loadSingleRowById(
    id: number | string,
    action: ApiActionsType = ApiActionsType.AddAction
  ) {
    this.isLoadingResults = true;
  ...
  loadListData(callback?: () => void) {
    ...
    this.sub.add(
      this.apiDatasource.getFilteredElements(params).subscribe(
        (data: T[] | ApiPaginatorListResponse<T>) => {
          if (data["totalCount"]) {
  ...
  openDetail(el: T) {
    if (this.subscribeRoute) {
      this.router.navigate([
        this.currentPath,
        el ? this.apiDatasource.apiIdExtractor(el) : "new",
      ]);
    } else {
      ...
      dialogRef.afterClosed().subscribe((result: T) => {
        let callback = () => {
          if (this.selectedElement && this.dataSourceArray) {
            ...
            const s = this.dataSourceArray.find(
              (el) =>
                this.idExtractor(el) == this.idExtractor(this.selectedElement)
            );
  ...
  onItemSelected(item: T) {
    if (
      this.selectedElement == null ||
      this.idExtractor(this.selectedElement) != this.idExtractor(item)
    ) {
  ...
  @ViewChild("searchHost", { static: true, read: ViewContainerRef })
  searchHost: ViewContainerRef;
  ...
  setFormFilterSettings() {
    ...
    this.sub.add(
      this.searchForm.onFilterChanged.subscribe((_: string) => {
  ...
  openStampaDialog() {
    ...
    this.sub.add(
      dialogRef.afterClosed().subscribe((result: StampaModalResponse) => {
        if (result) {
  ...
  saveListOrder(list: T[], path: string = "updateorder") {
    if (this.isAuthorizedToModify()) {
      this.apiDatasource.updateListOrder(list, path).subscribe(
        (res) => {
          console.log(res);
  ...
  dropItem(event: CdkDragDrop<string[]>) {
    if (event.previousContainer === event.container) {
      this.prepareSingleListToOrderSave(
        event.previousIndex,
        event.currentIndex,
        this.ordineFieldName
      );
    } else {
      this.prepareMultipleListToOrderSave(
  ...
  prepareMultipleListToOrderSave(
    previousContainerData: string[],
   ...
   */
  prepareSingleListToOrderSave(
    previousIndex: number,
    currentIndex: number,
    ordineFieldName: string = "Ordine"
  ) {

 ...
 */
@Directive()
export abstract class RicercaFieldAbstractComponent
  ...
  refreshValue() {
    if (this.selectedOperatore != null && this.fieldStringValue != null) {
      if (this.field.actualFilterMap) {
        this.value = this.field.actualFilterMap(
          this.field.Name,
          this.selectedOperatore,
          this.fieldStringValue
        );
      } else {
        const name =
          this.field.parentReference != null
            ? this.field.parentReference + "." + this.field.Name
            : this.field.Name;
        this.value =
          name + "." + this.selectedOperatore + "=" + this.fieldStringValue;
      }
    }
  }

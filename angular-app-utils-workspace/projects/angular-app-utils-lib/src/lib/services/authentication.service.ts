...
export abstract class AuthenticationService<LoginInfo> {
     ...
     */
    loginWithOtp(username: string, password: string, secret: string, remeberBrowser: boolean): Observable<LoginInfo> {
        return this.http.post<LoginInfo>(`${this.apiUrl}login`, { Username: username, Password: password, TempSecret: secret, SafePlace: remeberBrowser }, { headers: this.httpHeaders }).pipe(
            ...
            localStorage.setItem(TOKEN_OTP + '_' + username, remeberBrowser ? secret : '#');
    ...
    resetPassword(data: ResetPwdDialogData): Observable<LoginInfo> {
        return this.http.post<LoginInfo>(`${this.apiUrl}login/resetpwd`,
            { Email: data.username, OldPassword: data.oldPassword, Password: data.password }, { headers: this.httpHeaders })
            .pipe(map((utente) => {
                if (localStorage.getItem(CURRENT_USER) != null) {
                    localStorage.removeItem(CURRENT_USER);
                    //localStorage.setItem(CURRENT_USER, JSON.stringify(utente));
                }
                this.currentLoginInfoSubject.value['DataScadenza'] = utente['DataScadenza'];
    ...
    forgotPassword(username: string): Observable<boolean> {
        ...
        return this.http.post<boolean>(`${this.apiUrl}login/forgotpwd?${param}`,
            null, { headers: this.httpHeaders });
    }

    resetTemporaryPassword(username: string): Observable<ResetPwdData> {
        const param = `username=${username}`;
        return this.http.post<ResetPwdData>(`${this.apiUrl}login/tmpcrd?${param}`, null, { headers: this.httpHeaders });
    }
